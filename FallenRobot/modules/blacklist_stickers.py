    ...
    for text in split_text:
        if sticker_list == "<b>Lista de stickers en la lista negra actualmente en {}:</b>\n".format(
            ...
            send_message(
                ...
                "No hay stickers en la lista negra en <b>{}</b>!".format(
    ...
    if len(words) > 1:
        ...
        for trigger in to_blacklist:
            try:
                ...
                send_message(
                    update.effective_message,
                    "Sticker `{}` no pudo ser encontrado!".format(trigger),
                    parse_mode="markdown",
                )
        ...
        if len(to_blacklist) == 1:
            send_message(
                update.effective_message,
                "Sticker <code>{}</code> añadido a la lista negra de stickers en <b>{}</b>!".format(
            ...
            send_message(
                update.effective_message,
                "<code>{}</code> stickers añadidos a la lista negra <b>{}</b>!".format(
        ...
        if trigger is None:
            send_message(update.effective_message, "Sticker inválido!")
            return
        try:
            bot.getStickerSet(trigger)
            sql.add_to_stickers(chat_id, trigger.lower())
        ...
        send_message(
            update.effective_message,
            "Dime qué stickers quieres agregar a la lista negra.",
        )
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                ...
                send_message(
                    update.effective_message, "Este sticker no está en la lista negra....!"
                )
            ...
            send_message(
                update.effective_message,
                "Ninguna de estos stickers existen, por lo que no se pueden quitar..",
                parse_mode=ParseMode.HTML,
            )
            ...
            send_message(
                update.effective_message,
                "Sticker <code>{}</code> eliminado de la lista negra. {} no existía, por lo que no se elimina.".format(
        ...
        if success:
            ...
            send_message(
                update.effective_message,
                "{} no se encuentra stickers de la lista negra...!".format(trigger),
            )
    else:
        send_message(
            update.effective_message,
...
def blacklist_mode(update: Update, context: CallbackContext):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            send_message(
                update.effective_message, "Puedes usar este comando en grupos, no en privado"
            )
            return ""
    ...
    if args:
        if args[0].lower() in ["off", "nothing", "no"]:
            ...
            if len(args) == 1:
                teks = """Parece que está intentando establecer un valor temporal en la lista negra, pero no ha determinado el tiempo; usar `/blstickermode tban <timevalue>`.
                                          Ejemplos de valores de tiempo: 4m = 4 minutos, 3h = 3 horas, 6d = 6 dias, 5w = 5 semanas."""
            ...
            if len(args) == 1:
                teks = """Parece que está intentando establecer un valor temporal en la lista negra, pero no ha determinado el tiempo; usar `/blstickermode tmute <timevalue>`.
                                          Ejemplos de valores de tiempo: 4m = 4 minutos, 3h = 3 horas, 6d = 6 dias, 5w = 5 semanas."""
                send_message(update.effective_message, teks, parse_mode="markdown")
                return
            ...
            send_message(
                update.effective_message,
                "Sólo entiendo off/del/warn/ban/kick/mute/tban/tmute!",
            )
            return
        if conn:
            text = "El modo de stickers de la lista negra cambió, los usuarios serán `{}` at *{}*!".format(
                settypeblacklist, chat_name
            )
        else:
            text = "El modo de stickers de la lista negra cambió, los usuarios serán `{}`!".format(
                settypeblacklist
            )
        ...
        return (
            "<b>{}:</b>\n"
            "<b>Admin:</b> {}\n"
            "Se cambió el modo de lista negra de stickers, los usuarios serán {}.".format(
        ...
        if conn:
            text = "El modo de stickers de la lista negra está configurado actualmente en *{}* in *{}*.".format(
                settypeblacklist, chat_name
            )
        else:
            text = "El modo de stickers de la lista negra está configurado actualmente en *{}*.".format(
                settypeblacklist
            )
            ...
            try:
                if getmode == 0:
                    ...
                    warn(
                        ...
                        chat,
                        "Usando sticker '{}' que está en la lista negra de stickers".format(
                            trigger
                        ),
                        message,
                        update.effective_user,
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} silenciado porque está usando '{}' que en la lista negra de stickers".format(
                    ...
                    if res:
                        bot.sendMessage(
                            chat.id,
                            "{} pateado porque está usando '{}' que está en la lista negra de stickers".format(
                                mention_markdown(user.id, user.first_name), trigger
                            ),
                            parse_mode="markdown",
                        )
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} banned porque está usando '{}' que está en la lista negra de stickers".format(
                            mention_markdown(user.id, user.first_name), trigger
                        ),
                        parse_mode="markdown",
                    )
                    ...
                    bot.sendMessage(
                        chat.id,
                        "{} silenciado por {} porque usa '{}' que está en la lista negra de stickers".format(
                            mention_markdown(user.id, user.first_name), value, trigger
                        ),
                        parse_mode="markdown",
                    )
