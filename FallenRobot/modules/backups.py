...
def import_data(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        try:
            if str(context.bot.id) != str(data[str(chat.id)]["bot"]):
                return msg.reply_text(
                    "La copia de seguridad de otro bot que no se sugiere puede causar el problema; es posible que los documentos, las fotos, los videos, los audios y los registros no funcionen como deberían."
                )
        except Exception:
            pass
        # Select data source
        ...
        try:
            ...
            msg.reply_text(
                f"Ocurrió un error al recuperar sus datos. El proceso fracasó. Si tiene algún problema con esto, llévelo a @{SUPPORT_CHAT}"
            )

            LOGGER.exception(
                "Falló la impresión del chat %s con el nombre %s .",
                str(chat.id),
                str(chat.title),
            )
            return
...
def export_data(update, context):
    ...
    if checkchat.get("status"):
        if jam <= int(checkchat.get("value")):
            ...
            update.effective_message.reply_text(
                "Solo puedes hacer una copia de seguridad una vez al día!\nPuede hacer una copia de seguridad nuevamente en aproximadamente `{}`".format(
                    timeformatt
                ),
    ...
    context.bot.sendDocument(
        ...
        caption="📤*Copia de seguridad exportada con éxito:*\nChat: `{}`\nChat ID: `{}`\nEn: `{}`\n\nNota: Este `FallenRobot-Backup` fue hecho especialmente para notas 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
...
__help__ = """
*Sólo para el propietario del grupo:*
